...
export const AddEditTenantModal: React.FC<AddEditTenantModalProps> = ({
  ...
  const handleAddPeriod = () => {
    const startYr = parseInt((contractStart || '').split('-').pop() || '2026', 10) || 2026;
    const nextYear = startYr + scheduledPeriods.length;
    const lastRent = scheduledPeriods[scheduledPeriods.length - 1]?.monthlyRent || fixedMonthlyRent || 15000;
    const newPeriod: RentScheduleItem = {
      id: String(Date.now()),
      fromMonth: `01-Jan-${nextYear}`,
      toMonth: `31-Dec-${nextYear}`,
      monthlyRent: Math.round(lastRent * 1.05),
    };
      ...
      <div className="bg-[#F1F5F9] border-2 border-[#475569] rounded-[4px] shadow-2xl w-[720px] max-w-full max-h-[90vh] flex flex-col overflow-hidden text-[12px] font-sans">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-[#F8FAFC]">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    <button
                      ...
                      className="px-2.5 py-1 bg-white hover:bg-slate-100 border border-slate-300 text-slate-700 text-[11px] font-medium rounded-[2px] flex items-center gap-1 shadow-xs cursor-pointer"
                    >
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-[#CBD5E1]">
            ...
            <button
              type="submit"
              className="px-5 py-1.5 bg-[#2563EB] text-white font-semibold text-[12px] rounded-[3px] border border-[#1D4ED8] hover:bg-[#1D4ED8] shadow-sm flex items-center gap-1.5 min-w-[90px] cursor-pointer"
            >
